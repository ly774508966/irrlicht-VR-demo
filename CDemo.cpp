 ...
 timeForThisScene(0)
{
	...
	glove=p5glove_open();
	while (glove == NULL) {
		printf("%s: Can't open glove interface\n");
        glove=p5glove_open();
	}
	lastfire = 0;
	glovedelay = 0;
...
*/
    ...
    int total = 0;
		...
		for (i = 0; i < 8; i++) {
            ...
            if ( glovey > info.ir[i].y ) glovey = info.ir[i].y;


            glovex = glovex + info.ir[i].x;
            glovey = glovey + info.ir[i].y;
            glovez = glovez + info.ir[i].z;
            total = i;
//			printf("%d:(%4d,%4d,%4d) \n ",i, info.ir[i].x,info.ir[i].y,info.ir[i].z);
		}
		...
		if ( (  glovez < calibratez -60 ) ||  (  glovez >  calibratez +60 )  ) {

            core::vector3df  pos = camera->getPosition();
            core::vector3df  dest;
            if (  glovez < calibratez -60 ) {
                printf("Traz \n" );
                dest = pos + (camera->getTarget() - pos).normalize() * -4.0f;
		    } else  {
                printf("Frente \n" );
                dest  = pos + (camera->getTarget() - pos).normalize() * 4.0f;
		    }
        ...
        if ( (  glovey   <  calibratey -50 ) || (  glovey >  calibratey +50 )  ){
       ...
       if ( info.finger[P5GLOVE_INDEX] > 25 ) {
          shoot();
	      printf("Fire!");
       }

        if ( info.finger[P5GLOVE_PINKY] > 25 ) {
           printf("Calibrated! \n" );
          calibratex = glovex;
          calibratey = glovey;
          calibratez = glovez;
...
/*
	        camera = sm->addCameraSceneNodeFPS(
	              sm->getRootSceneNode(),            // node raiz
...
void CDemo::shoot()
{
     ...
     int timenow = device->getTimer()->getTime();
     if (timenow < lastfire + reload_time  )  return;
